...
举例：某验三代（非最新四代，三代具有说服力）

这里的只是简单的，还有一些消消乐，五子棋什么的，没这些，大致思路是一样的。

还有，别管他是滑块，还是一键通过的三代，只管看思路分析就行了，主要是学习。


:::tip
四代不仅仅会有加密算法，还比三代更难，所以这里以三代举例，适当加入了加密算法，在学完加密后，举例四代分析。
:::


三代的三个w是互相关联的，因此，并不是绕过哪个参数，就能怎么样了。

这不仅仅为了破解而破解啊，主要是为了养成逆向的思路。

思路是分析80%，真正去做10%，运气10%

目标 -> 分析 -> 拆出来代码运行 -> 编写模拟脚本 
...
<img alt="XSS攻击方案" src={useBaseUrl('/images/rsecu/webjs/2_16_1.png')} />
...
<img alt="XSS攻击方案" src={useBaseUrl('/images/rsecu/webjs/2_16_2.png')} />
...
<img alt="XSS攻击方案" src={useBaseUrl('/images/rsecu/webjs/2_16_3.png')} />

响应预览中返回了一些 js 文件及对应的版本号：

<img alt="XSS攻击方案" src={useBaseUrl('/images/rsecu/webjs/2_16_4.png')} />

第一个get.php?xxx，url 中传递了一些参数，关键部分如下：
...
<img alt="XSS攻击方案" src={useBaseUrl('/images/rsecu/webjs/2_16_5.png')} />

响应内容如下，这里没什么需要注意的，feedback 就是某验的帮助中心：

<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_6.png')} />

2.点击按钮进行验证之后，Network 中抓包到了以下信息：

<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_7.png')} />
...
<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_8.png')} />

响应返回验证码模式，滑块验证码为 slide，点选验证码为 click：

<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_9.png')} />
...
<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_10.png')} />

这个响应返回了很多关键内容：

bg：被打乱的带缺口背景图，需要还原，372fe236d.webp

<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_11.png')} />

fullbg：被打乱的完整背景图，需要还原，7bfaaa72b.webp

<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_12.png')} />

slice：滑块图片，不需要还原，372fe236d.png

<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_13.png')} />

c：关键参数，与后面 aa 参数的值有关，固定值；
s：关键参数，与后面 aa 参数的值有关。

<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_14.png')} />
...
<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_15.png')} />

滑动滑块验证通过即会返回以下内容：

<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_16.png')} />

失败则会返回：

<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_17.png')} />
...
<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_18.png')} />

进入后点击左下角 {} 大括号，格式化文件，再 ctrl + f  局部搜索 "\u0077"，只有一个结果，在第 6086 行，在第 6088 行打下断点，滑动滑块即会断住，h + u 即为 w 参数的值：

<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_19.png')} />

2.通过 Initiator 跟栈，跟进到 $_CId 中：

<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_20.png')} />

进去同样格式化后，会跳转到第 4583 行，在该行打下断点：

<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_21.png')} />

向上跟栈到 $_CCBv 中同样会找到刚刚的位置：

<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_22.png')} />
...
<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_23.png')} />

会跳转到第 6218 行，在 6227 行 return 处打下断点，重新拖动滑块，即会断住：

<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_24.png')} />
...
<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_25.png')} />
...
<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_26.png')} />

跳转到第 6208 行，在第 6214 行 return 处打下断点：

<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_27.png')} />

Ot 即 16 为字符串，Ot = rt()，跟进到 rt 函数中，在第 4213 行，于第 4219 行打下断点后会发现，16 位字符串是由四个 t() 方法的结果相加得到的：

<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_28.png')} />

跟进到 t() 方法的定义位置，在第 4203 行，第 4208 行即为随机字符串算法：

<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_29.png')} />
...
`this["$_CCEc"]` 分析完了，那 `new U()["encrypt"]` 是什么呢，选中 `new U()` 后，从原型链中可以看到 `setPublic`，根据经验很有可能就是 RSA 加密设置公钥。

<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_30.png')} />

跟进去查看一下：

<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_31.png')} />


跳转到第 2908 行，在第 2922 行断住后，$_BACAs(332) 的值为 "Invalid RSA public key"：

<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_32.png')} />
...
<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_33.png')} />
...
<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_34.png')} />

搜索后可知，其定义在第 128 行：

<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_35.png')} />
...
<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_36.png')} />

定义在第 7 行，直接将 QBLnx.$_Ak 整个扣下来即可，然后会报错提示，window 和 ht 未定义，ht 为 navigator：

<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_37.png')} />
...
<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_38.png')} />

通过搜索，其定义在第 68 行，将 QBLnx.$_BP 整个扣下来即可，至此，u 参数成功复现：

<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_39.png')} />
<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_40.png')} />
...
<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_41.png')} />

将这里写成跟之前一样的固定值， `gt["stringify"](o)` 返回的是 JSON 格式的数据，由 o 参数生成：

<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_42.png')} />

对比分析以下 o 参数中，哪些是定值，哪些是动态变化的，可以看到箭头所指的值都不一样了：

<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_43.png')} />
...
<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_44.png')} />
...
<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_45.png')} />
...
<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_46.png')} />
...
<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_47.png')} />

n[$_DAAAV(871)] 为 passtime 值，定义在第 8164 行，为滑动结束时间 - 开始时间：

<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_48.png')} />
接下来分析下 aa 参数，其定义在第 6017 行，值由参数 e 传递，同样向上跟栈到 $_CGlj 中，为第 8168 行的 l 值，l 定义在第 8167 行，三个参数加密后得到：

<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_49.png')} />
...
<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_50.png')} />

接下来跟进到 `n[$_DAAAV(913)][$_CJJJb(1066)]` 中，分析下轨迹是如何加密的，在第 4065 行，于第 4133 行打下断点，第 4108 行的 `this[$_BEHAL(343)]` 即为轨迹值，关于轨迹算法后面会专门出一期文章：

<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_51.png')} />
...
<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_52.png')} />

ct 定义在第 4223 行，扣下来，报错提示，`(intermediate value)[$_BEHAL(...)] is not a function：`

<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_53.png')} />
...
<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_54.png')} />
...
<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_55.png')} />

aa 参数分析完成，接下来分析 rp 参数，定义在第 6076 行：

<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_56.png')} />
...
<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_57.png')} />

这里是 MD5 加密，也可以直接通过引库复现：

<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_58.png')} />

ep 定义在第 6018 行，跟进到 r[$_CAHJd(714)] 中，参数定义在第 6239 行：

<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_59.png')} />

跟进 `new bt()[$_CBGEC(760)]` 中，在第 5268 行打下断点，结果如下：

<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_60.png')} />

o 中这个键值对内容每天都会变化：

<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_61.png')} />


在第 6021 行打下断点，此时的 o 中还没有生成以上键值对：

<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_62.png')} />
...
<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_63.png')} />

在该文件的第 1253 行打下断点，可以看到此时的 t 中已经生成了 h9s9: "1803797734" ：

<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_64.png')} />

跟进到 StJC 方法中，其定义在第 1202 行，在第 1208 行打下断点，键值对在此处生成，可以通过全局导出调用：

<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_65.png')} />

至此，参数 o 复现完毕，回到第 6078 行，分析完 `V[$_CAIAt(353)]` `l` 即完成，跟进，定义在第 3218 行，在第 3230 行打下断点，这里为 AES 加密，初始向量 iv 值为 "0000000000000000"：

<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_66.png')} />
...
<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_67.png')} />

l 参数分析完毕，终于只剩下一个 h 了，`m[$_CAIAt(782)](l)` 即将 l 加密后得到的，跟进 `m[$_CAIAt(782)]`，定义在第 1568 行，在第 1575 行打下断点，为 e 中两个 value 值相加：

<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_68.png')} />

e 定义在第 1574 行，t 为传入的 l 参数，跟进到 this[$_GFJn(264)] 中，在第 1523 行，复现如下：

<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_69.png')} />

校验结果一致：

<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_70.png')} />
...
:::tip
Canvas是前端JS必备的知识，可以去网上搜一下学一下，很简单几个API就会了，主要用于绘制。
:::

<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_71.png')} />

点击按钮进行验证即会断住，格式化后跳转到第 295 行，

<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_72.png')} />
...
```
:::tip
反混淆的话，去网上搜一下在线JS工具即可，如果自己有想法，可以安装babel进行更精细的反混淆，也可以自己利用babel写个反混淆工具，希望做个工具哈，如果有做工具的兴趣可以沟通我。
:::

这里就是 Canvas 绘图的过程，关键乱序算法部分在 `QBLnx.$_Db()[12][17]` 中：
...
<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_73.png')} />
...
<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_74.png')} />
...
<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_75.png')} />
...
还原后如下：
```
<img alt="feedback" src={useBaseUrl('/images/rsecu/webjs/2_16_76.png')} />
...
执行geetest_xxxx ,xxx是上面的值即可。

这里强调一点的是，不可能存在成功100%的情况，因为任何验证工具会故意弄一些错的验证，造成了计算出来的结果和程序的不符合。

## 总结思路

某验不管是几代，都是这种思路，具体为

分析，把代码拿出来到VSCODE里面，运行一下，如果报错了，那么就去看看哪里错了，是不是函数缺少了，基本都是函数缺少的问题，然后最后是找找他的一些算法自我一般都是RSA，AES，MD5不会有太复杂的，碰到太难的算法的话，会想其他方法了，如人工智能跑算法，不过成本巨大，一般不去用人工智能。然后最后就是拼图把图弄出来，最后用Python代码模拟一下操作即可。