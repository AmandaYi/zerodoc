...
分析，把代码拿出来到VSCODE里面，运行一下，如果报错了，那么就去看看哪里错了，是不是函数缺少了，基本都是函数缺少的问题，然后最后是找找他的一些算法自我一般都是RSA，AES，MD5不会有太复杂的，碰到太难的算法的话，会想其他方法了，如人工智能，不过成本巨大，一般不去用人工智能。然后最后就是拼图把图弄出来，最后用Python代码模拟一下操作即可。